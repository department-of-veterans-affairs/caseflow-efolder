describe V2::DownloadManifestJob do
  context "#perform" do
    ...
    context "when document list is empty" do
      before do
        allow(Efolder::Fakes::DocumentService).to receive(:v2_fetch_documents_for).and_return([])
      end
    ...
    context "when document list is not empty" do
      before do
        allow(Efolder::Fakes::DocumentService).to receive(:v2_fetch_documents_for).and_return(documents)
      end
