describe RecordFetcher do
  ...
  context "#process" do
    ...
    context "when file is not in S3" do
      before do
        allow(S3Service).to receive(:fetch_content).and_return(nil)
        allow(Caseflow::Fakes::DocumentService).to receive(:v2_fetch_document_file).and_return(fake_pdf_content)
      end
      ...
      context "when VBMS returns a tiff file" do
        ...
        before do
          ...
          allow(Caseflow::Fakes::DocumentService).to receive(:v2_fetch_document_file).and_return(tiff_content)
