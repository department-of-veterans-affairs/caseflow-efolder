describe "Manifests API v2", type: :request do
  ...
  context "View download history" do
    let(:manifest1) { Manifest.find_or_create_by!(user: user, file_number: "123C") }
    let(:manifest2) { Manifest.find_or_create_by!(user: user, file_number: "567C") }
    let(:manifest3) { Manifest.find_or_create_by!(user: user, file_number: "897C") }
    let(:manifest4) { Manifest.find_or_create_by!(user: user, file_number: "935C") }
